...
const formSchema = z.object({
	...
	minutes: z.string().transform((val) => parseInt(val, 10)).refine(val => !isNaN(val) && val >= 0, {
		message: "Minutes must be a non-negative integer.",
	}),
	seconds: z.string().transform((val) => parseInt(val, 10)).refine(val => !isNaN(val) && val >= 0, {
	...
	return (
		<Form {...form}>
			<form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
				...
				/>
				<Button className="bg-summer-dog text-zin-950 w-full" type="submit">Submit</Button>
			</form>
		</Form>
	);
};
